...
public class AudioManager : Singleton<AudioManager>
{
    ...
    {
        GameManager.Instance.EventChangeSettings += ChangeSettings;
    }
    ...
    public void ChangeSettings()
    {
        foreach (var item in sources)
        {
            ...
            if (musicIDs.Exists(x => x == item.clip.name))            //Если МУЗЫКА
            {
                item.mute = !GameManager.Instance.settingSound;
            }
            else                                                        //Если ЗВУК
            {
                item.mute = !GameManager.Instance.settingSound;
    ...
    private void Play(AudioSource _source, bool _isSound, bool _loop, float _volume)
    {
        ...
        {
            _source.mute = !GameManager.Instance.settingSound;
        }
        else                                //Если МУЗЫКА
        {
            _source.mute = !GameManager.Instance.settingSound;
